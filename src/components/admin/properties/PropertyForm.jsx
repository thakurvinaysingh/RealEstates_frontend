...
const PropertyForm = ({ propertyId, isEdit = false }) => {
  ...
  // local file uploads (images & gallery)
  ...
  const loadProperty = async () => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...
        financialTerms: property.financialTerms || prev.financialTerms,
        owner: property.owner || prev.owner,
  ...
  const validate = () => {
    ...
    if (!Number(formData.totalUnits) || Number(formData.totalUnits) <= 0) e.totalUnits = 'Total Units must be greater than 0.';
    if (!Number(formData.slotsCount) || Number(formData.slotsCount) <= 0) e.slotsCount = 'Slots Count must be greater than 0.';
    if (!Number(formData.totalValue) || Number(formData.totalValue) <= 0) e.totalValue = 'Total Value must be greater than 0.';
    if (formData.startDate && formData.endDate) {
      const s = new Date(formData.startDate), eDate = new Date(formData.endDate);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const hasFiles = (imagesFiles?.length || 0) > 0 || (galleryFiles?.length || 0) > 0;
      ...
      // If files selected, send as multipart/form-data
      if (hasFiles) {
        const fd = new FormData();

        // Scalars & simple fields
        Object.entries(basePayload).forEach(([k, v]) => {
          if (['images','gallery','financialTerms','tieredReturn','capitalGrowthSplit','owner','faqs','reasonsToInvest','occupancyOptions','keyUpdates','reports'].includes(k)) return;
          fd.append(k, v ?? '');
        });

        // Arrays/objects as JSON strings (backend parses JSON strings)
        ...
        // Also include any URL entries (comma-separated, backend supports)
        if ((formData.images || []).length) fd.append('images', (formData.images || []).join(','));
        if ((formData.gallery || []).length) fd.append('gallery', (formData.gallery || []).join(','));

        if (isEdit && propertyId) {
          await apiService.updateProperty(propertyId, fd);
          toast({ title: 'Success', description: 'Property updated successfully' });
        } else {
          await apiService.createProperty(fd);
          toast({ title: 'Success', description: 'Property created successfully' });
        }
      } else {
        // JSON body
        if (isEdit && propertyId) {
          await apiService.updateProperty(propertyId, basePayload);
          toast({ title: 'Success', description: 'Property updated successfully' });
        } else {
          await apiService.createProperty(basePayload);
          toast({ title: 'Success', description: 'Property created successfully' });
        }
      }
  ...
  const addGalleryImage = () => {
    if (newGalleryImage.trim()) {
      setFormData(prev => ({ ...prev, gallery: [...prev.gallery, newGalleryImage.trim()] }));
      setNewGalleryImage('');
    }
  };
  const removeGalleryImage = (index) => setFormData(prev => ({ ...prev, gallery: prev.gallery.filter((_, i) => i !== index) }));
  const addReason = () => {
    if (newReason.trim()) {
      setFormData(prev => ({ ...prev, reasonsToInvest: [...prev.reasonsToInvest, newReason.trim()] }));
      setNewReason('');
    }
  };
  const removeReason = (index) => setFormData(prev => ({ ...prev, reasonsToInvest: prev.reasonsToInvest.filter((_, i) => i !== index) }));
  const addOccupancyOption = () => {
    if (newOccupancyOption.trim()) {
      setFormData(prev => ({ ...prev, occupancyOptions: [...prev.occupancyOptions, newOccupancyOption.trim()] }));
      setNewOccupancyOption('');
    }
  };
  const removeOccupancyOption = (index) => setFormData(prev => ({ ...prev, occupancyOptions: prev.occupancyOptions.filter((_, i) => i !== index) }));
  const addFaq = () => {
  ...
  const addTieredReturn = () => {
    if (newTieredReturn.range.trim() && newTieredReturn.rate.trim()) {
      setFormData(prev => ({ ...prev, tieredReturn: [...prev.tieredReturn, { ...newTieredReturn }] }));
      setNewTieredReturn({ range: '', rate: '' });
    }
  };
  const removeTieredReturn = (index) => setFormData(prev => ({ ...prev, tieredReturn: prev.tieredReturn.filter((_, i) => i !== index) }));
  const addCapitalGrowthSplit = () => {
    if (newCapitalGrowthSplit.party.trim() && newCapitalGrowthSplit.pct.trim()) {
      setFormData(prev => ({ ...prev, capitalGrowthSplit: [...prev.capitalGrowthSplit, { ...newCapitalGrowthSplit }] }));
      setNewCapitalGrowthSplit({ party: '', pct: '' });
    }
  };
  const removeCapitalGrowthSplit = (index) => setFormData(prev => ({ ...prev, capitalGrowthSplit: prev.capitalGrowthSplit.filter((_, i) => i !== index) }));
  const addKeyUpdate = () => {
    if (newKeyUpdate.text.trim()) {
      setFormData(prev => ({
        ...prev,
        keyUpdates: [...prev.keyUpdates, { ...newKeyUpdate, date: newKeyUpdate.date ? new Date(newKeyUpdate.date) : new Date() }]
      }));
      setNewKeyUpdate({ date: '', text: '', link: '' });
    }
  };
  const removeKeyUpdate = (index) => setFormData(prev => ({ ...prev, keyUpdates: prev.keyUpdates.filter((_, i) => i !== index) }));
  const addReport = () => {
    if (newReport.title.trim() && newReport.url.trim()) {
      setFormData(prev => ({ ...prev, reports: [...prev.reports, { ...newReport }] }));
      setNewReport({ title: '', desc: '', label: '', url: '' });
    }
  };
  const removeReport = (index) => setFormData(prev => ({ ...prev, reports: prev.reports.filter((_, i) => i !== index) }));
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-gray-500">Fields marked with <span className="text-red-600">*</span> are required.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1">
              <Label htmlFor="title" className="text-blue-900">Property Title <span className="text-red-600">*</span></Label>
              <Input
                id="title"
                placeholder="e.g., Greenwood Plaza – Tower A"
                value={formData.title}
                onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))}
                required
                className={`rounded-lg ${errors.title ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              />
              {errors.title && <p className="text-xs text-red-600">{errors.title}</p>}
              <p className="text-xs text-gray-500">A short, clear name for the property.</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              />
              <p className="text-xs text-gray-500">Full postal address (optional).</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                className="rounded-lg"
              />
              <p className="text-xs text-gray-500">City / Region to help users find this property.</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-gray-500">Choose the most accurate category.</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                className={`rounded-lg ${errors.endDate ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              />
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-1">
              <Label htmlFor="totalUnits" className="text-blue-900">Total Units <span className="text-red-600">*</span></Label>
              <Input
                id="totalUnits"
                type="number"
                ...
                min="1"
                className={`rounded-lg ${errors.totalUnits ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              />
              {errors.totalUnits && <p className="text-xs text-red-600">{errors.totalUnits}</p>}
              <p className="text-xs text-gray-500">Total fractional units for this asset.</p>
            </div>

            <div className="space-y-1">
              <Label htmlFor="slotsCount" className="text-blue-900">Slots Count <span className="text-red-600">*</span></Label>
              <Input
                id="slotsCount"
                type="number"
                placeholder="e.g., 20"
                ...
                min="1"
                className={`rounded-lg ${errors.slotsCount ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              />
              {errors.slotsCount && <p className="text-xs text-red-600">{errors.slotsCount}</p>}
              <p className="text-xs text-gray-500">How many slots investors can purchase.</p>
            </div>
            ...
            <div className="space-y-1">
              <Label htmlFor="totalValue" className="text-blue-900">Total Value ($) <span className="text-red-600">*</span></Label>
              <Input
                id="totalValue"
                type="number"
                ...
                min="1"
                className={`rounded-lg ${errors.totalValue ? 'border-red-500 focus-visible:ring-red-500' : ''}`}
              />
              {errors.totalValue && <p className="text-xs text-red-600">{errors.totalValue}</p>}
              <p className="text-xs text-gray-500">Full market value of the property.</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <Textarea
                ...
                placeholder="High-level summary of the project and strategy"
                value={formData.projectDescription}
                onChange={(e) => setFormData(prev => ({ ...prev, projectDescription: e.target.value }))}
                rows={4}
                className="rounded-lg"
              />
              <p className="text-xs text-gray-500">What makes this property compelling?</p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Textarea
                ...
                placeholder="Key terms and investor benefits"
            ...
            <div className="space-y-1">
              ...
              <Textarea
                id="risks"
                placeholder="Outline potential risks"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1 md:col-span-2">
              ...
              <Textarea
                id="ownerBio"
                placeholder="Brief background of the owner"
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-1">
              ...
              <Input
                id="security"
                placeholder="e.g., 1st-Rank Mortgage"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <Label className="text-blue-900">Upload from computer</Label>
                  <Input
                    type="file"
                    multiple
                    accept="image/*"
                  ...
                  {imagesFiles.length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">{imagesFiles.length} file(s) selected</p>
                  )}
                  <p className="text-xs text-gray-500 mt-1">Add URLs or select files. Both will be uploaded.</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div>
                  ...
                  {galleryFiles.length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">{galleryFiles.length} file(s) selected</p>
                  )}
                  <p className="text-xs text-gray-500 mt-1">Optional extra images for the gallery.</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Input
                placeholder="Short benefit point (e.g., Prime location)"
                value={newReason}
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Input
                placeholder="e.g., Owner-occupied / Tenanted"
